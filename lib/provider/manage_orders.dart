...
class ManageOrdersScreen extends StatelessWidget {
  ...
  final CollectionReference ordersRef = const FirebaseFirestore().collection('orders');

  Future<void> _updateStatus(String id, OrderStatus s) async {
    await FirebaseFirestore.instance.collection('orders').doc(id).update({'status': orderStatusToString(s)});
  }
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            ...List.from(d['items'] ?? []).map((it) => ListTile(title: Text(it['name'] ?? ''), subtitle: Text('Qty: ${it['qty']}'))),
            const Divider(),
  ...
  Widget build(BuildContext context) {
    final ordersStream = FirebaseFirestore.instance.collection('orders').orderBy('createdAt', descending: true).snapshots();
    return Scaffold(
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, i) {
              ...
              return Card(
                ...
                child: ListTile(
                  title: Text('Order ${d.id} — ${d['customerName'] ?? 'Customer'}'),
                  subtitle: Text('Total: \$${(d['total'] ?? 0).toString()} • Status: ${status.name}'),
                  trailing: PopupMenuButton<OrderStatus>(
                    onSelected: (s) => _updateStatus(d.id, s),
                    itemBuilder: (_) => OrderStatus.values.map((s) => PopupMenuItem(value: s, child: Text(s.name))).toList(),
                  ),
